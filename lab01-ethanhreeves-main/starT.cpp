...
string starT(int width, int height)
{
  ...
  if (height < 2 || width % 2 != 1 || width < 3)
      return result;
  
  int midWidth = width / 2; //number of spaces to the middle
    
  for (int i = 0; i < width; ++i) { //adding the top of the T
    result += "*";
  }
  result += "\n"; //newline!
  ...
  for (int i = 0; i < height - 1; ++i) {
    for (int y = 0; y < midWidth; ++y) {
      result += " "; //adding spaces to the middle
    }
    result += "*"; //adding the middle line
    for (int y = 0; y < midWidth; ++y) {
      result += " "; //adding spaces on the other side of the middle line
    }
    result += "\n"; //new line
  }
  return result;
